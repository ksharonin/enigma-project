...
public class MachineTest {
    ...
    @Test
    public void testConvertMsgMine() {
        Machine mach = mach1();
        mach.setPlugboard(new Permutation("(HQ) (EX) (IP) (TR) (BY)", AZ));
        assertEquals("QVPQSOKOILPUB", mach.convert("FROMHISSHOULD"));
    }

    @Test
    public void testPlugboard() {
        Machine mach = mach1();
        Alphabet alphy = new Alphabet("ABGFERT");
        Permutation ours = new Permutation("(AG)(TF)", alphy);
        mach.setPlugboard(ours);
        Permutation storeplugy = mach.plugboard();
        assertEquals(storeplugy, ours);
    }

    @Test(expected = EnigmaException.class)
    public void failingPlugboard() {
        Machine mach = mach1();
        mach.setPlugboard(new Permutation("(ABZ)(RF)", AZ));
    }

    @Test(expected = EnigmaException.class)
    public void failingPlugboardDueToChar() {
        Machine mach = mach1();
        mach.setPlugboard(new Permutation("(AB)(AF)", AZ));
    }
